...
void routineCallback(RTN rtn, void* v)
{
   ...
   else if (rtn_name == "CAPI_set_filter")
   {
      PROTO proto = PROTO_Allocate(PIN_PARG(CAPI_return_t),
            CALLINGSTD_DEFAULT,
            "CAPI_set_filter",
   ...
   else if (rtn_name == "CAPI_filter_incoming_messages")
   {
      PROTO proto = PROTO_Allocate(PIN_PARG(CAPI_return_t),
            CALLINGSTD_DEFAULT,
            "CAPI_filter_incoming_messages",
            PIN_PARG(CAPI_endpoint_t),
            PIN_PARG_END());

      RTN_ReplaceSignature(rtn,
            AFUNPTR(CAPI_filter_incoming_messages),
            IARG_PROTOTYPE, proto,
            IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
            IARG_END);
   }
   else if (rtn_name == "CAPI_dequeue_filtered_message")
   {
      PROTO proto = PROTO_Allocate(PIN_PARG(CAPI_return_t),
            CALLINGSTD_DEFAULT,
            "CAPI_dequeue_filtered_message",
            PIN_PARG(CAPI_endpoint_t),
            PIN_PARG(int),
            PIN_PARG(char*),
            PIN_PARG(int),
      ...
      RTN_ReplaceSignature(rtn,
            AFUNPTR(CAPI_dequeue_filtered_message),
            IARG_PROTOTYPE, proto,
            IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
            IARG_FUNCARG_ENTRYPOINT_VALUE, 1,
            IARG_FUNCARG_ENTRYPOINT_VALUE, 2,
